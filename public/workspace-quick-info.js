...
export class WorkspaceQuickInfo {
  ...
  async _loadGitMetadata(workspace) {
    ...
    // Prefer the on-disk path (sidebar passes projectPath) over the
    // internal workspace ID — the backend resolves either, but sessions
    // from other workspaces only carry a path, not a registered ID.
    if (workspace.workspacePath) {
      params.set("workspacePath", workspace.workspacePath);
    } else if (workspace.workspaceId) {
      params.set("workspaceId", workspace.workspaceId);
    }
