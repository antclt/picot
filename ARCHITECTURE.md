...
- `path-safety.ts` 提供带目录分隔符边界的 `path.resolve` containment 检查，
...
- **LAN surface 只读。** LAN/mobile 客户端可以加载静态资源并访问明确的
  ...
  `isLoopbackAddress` 与方法/路径表 `isLoopbackOnlyApiRequest` 做判断；缺少
  peer address 时 fail-closed。Node `http.createServer` 与 Bun `Bun.serve`
  必须使用同一策略。CORS 不是认证边界；移动端控制继续通过已认证 broker。
- **静态资源必须经过真实路径 containment。** `embedded-server.ts` 在 Node
  使用 `fs.realpath`、在 Bun 使用 `fs.promises.realpath`，并用
  `extensions/path-safety.ts::isPathWithinRoot` 重新校验 root 与 candidate。
  traversal、兄弟目录前缀（例如 `/public-assets`）和 symlink escape 必须拒绝。
- **路径规范化与系统打开器必须平台无关。** 前端使用
  `public/workspace/path-utils.js`；后端使用 `extensions/open-path.ts`，不使用
  `osascript` 或 shell 拼接；Rust `pi_manager.rs` 用 `PathBuf::join`，并在
  `HOME` / `USERPROFILE` / `HOMEDRIVE+HOMEPATH` 间回退查找 super-agent workspace。
...
- `bun run test` 跑 vitest（`public/**/*.test.js` 与 `extensions/**/*.test.ts`）加
  `scripts/check-tauri-permissions.js`。
- 安全与跨平台回归覆盖：`extensions/request-access.test.ts`（loopback 策略）、
  `extensions/path-safety.test.ts`（文件系统 containment）、
  `extensions/open-path.test.ts`（系统打开器）、`public/workspace/path-utils.test.js`
...
`workspace/file-browser.js`、`native/file-browser.js`、`file-tab-state.js`、
`file-language.js`、`file-preview-panel.js` 和 `sidebar/index.js` 共用这些
helpers。`FileBrowser.toMentionPath()` 对 Windows 路径按大小写不敏感的
segment 比较，并拒绝跨 drive 的路径。
...
- 前端 `ephemeral-chat-runtime.js`：仅接受匹配 instance+generation 的帧；`applySnapshot` 装载权威状态 + watermark；`applySequencedEvent` 要求严格 `lastAppliedSequence+1`，更大则暂停并 `ephemeral_snapshot_request` 重请求快照，重复序列抑制。非序列化的关联 `response` 仅用于请求/响应控制数据（当前为该运行时自己的可用模型列表），不进入渲染事件序列；模型命令使用 `provider` + `modelId`。broker 为每个临时 runtime 保留有界 journal（512 帧或 2 MiB），快照请求按 watermark 回放，缺口或溢出由 runtime 再次请求权威快照。
...
原生 owner 独占的多 tab 本地终端面板，位于右侧工作区下方，横跨 chat、File Preview、File Browser。每个 tab 拥有一个真实本地 shell（PTY），折叠面板或切换工作区时 PTY 后台继续运行，返回时从 checkpoint + journal 恢复屏幕。Terminal 与 File Browser 工具栏按钮使用统一的紧凑横向圆角描边样式；Side Chat 保留聊天气泡按钮。完整产品/交互/生命周期/安全契约见 [`docs/superpowers/specs/2026-07-21-terminal-panel-design.md`](docs/superpowers/specs/2026-07-21-terminal-panel-design.md)。
...
- 分区键是 `(ownerId, canonicalWorkspaceRoot)`；`terminalId` 仅在分区内有意义；`workspaceGeneration` 是 **attachment 授权令牌**（host 比较），**不是**保留分区键——否则返回后台工作区会丢 PTY。同路径的两个原生窗口因 owner 不同而完全隔离（不共享活终端/输出/配额）。
- 配额：每工作区 5 个非 closing tab 记录（含 failed/exited）；全局 15 个 live/reserved PTY（Running/Closing）。failed spawn 释放 live 配额但保留可见 failed tab 占 slot。
- capability 跨端口重注入：`capability_initialization_script` 改为 **loopback-agnostic**（仅检查 http loopback hostname，不硬编码端口）。capability 绑定 owner、跨 navigation 不变，因此跨端口 WebView reload 后新文档自动获得 capability，`client_hello` 正常认证。精确源边界仍由 `on_navigation`/`authorize_navigation` 硬门保证；init script 是纵深防御。capability 轮换仅在 `revoke_owner`（owner 销毁）。
- 方案选择依据：Tauri v2 无 public 运行时 init-script 注入 API（仅 builder 时 `initialization_script` + 页面内 `eval`），故未采用运行时注入；loopback-agnostic init script 是基于该事实确认后的实现选择，不削弱安全边界（on_navigation 硬门 + loopback guard + 256-bit 随机 token + broker workspace 授权多层防御保留）。

### 工作区转换、关闭与恢复

- 工作区转换：`TerminalPanel.beforeWorkspaceTransition()` 冻结交互 + checkpoint 所有 running tab，在 `transport.commitWorkspaceTransition` **之前**完成；提交后 `location.assign()` 销毁源文档，源 PTY 在源分区后台继续运行，新页面 `client_hello` 后 `terminal_list` 重建。取消调 `cancelWorkspaceTransition` 解锁。
- 关闭：前端 `TerminalPanel` 作为 `"terminal"` participant 注册进既有 `WindowCloseCoordinator`（`getCloseRisk`/`settleCloseRisk`/`setInteractionLocked`），version 4 risk 含 `terminalTabs`；Rust 是最终权威——`handle_window_destroyed` 在 `revoke_owner` 前调 `TerminalManager::kill_owner`，`RunEvent::Exit` 调 `kill_all`，即使 WebView 崩溃/未 settle。
- checkpoint/journal 重绑：`terminal_list` 返回 tab 描述符 + checkpoint + watermark + journal；前端先写 checkpoint，再按严格 `lastAppliedSequence+1` 应用 journal；前向缺口暂停并重请求 list；duplicate 序列忽略；溢出设持久 `historyGap` 警告但不杀 shell。
- 持久化：`terminal_state_store.rs` 按**规范化工作区根**键存 `terminal-state.json`（tab 顺序/label/profile/active index/panel 高度/schema 版本），**不含** output/checkpoint/pid/环境/title/owner/capability。这是跨重启的惰性元数据键，不是活终端所有权键；新 app run 用新 owner，首次展开按元数据重建 fresh shell 并显示一次性 restart notice。

### Shell profiles 与安全边界

- macOS：用户 `$SHELL`（若合法可执行），否则 `/bin/zsh`→`/bin/bash`，`-i` 交互启动，不插值用户文本。
- Windows：默认 Git Bash（`bin/bash.exe --login -i`，绝不 `git-bash.exe`），检测标准安装目录；缺失则可见失败 + 安装引导，**不**静默降级 PowerShell。PowerShell / Command Prompt 可手动选择。
- 终端即任意本地代码执行，故 native owner 验证强制：只有认证为当前原生窗口 owner 的 broker 连接可调用终端命令或接收终端事件；remote/LAN/移动/未验证/超时 client fail-closed。shell profile 是固定 host 定义（无任意 executable profile）。input/output/title/exit 文本/checkpoint/持久化 JSON 均按不可信有界处理；xterm + addon 是同源打包资产，无 CDN；capability 绝不进终端帧/错误/日志/持久化。
...
- 要改**LAN 边界、跨平台路径或静态资源服务**，同步更新本文件的
  `## 边界与不变量`、文件浏览安全边界和对应测试清单。
